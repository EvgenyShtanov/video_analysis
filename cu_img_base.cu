...
/* old version __device__ void img_rotate_full(IMGTYPE *img_out, int *w_out,
	int *h_out,IMGTYPE *img_in, int w_in, int h_in, float angle) {
	angle = angle ;
	...
	int w_full = w_in * fabs( cos(angle) ) + h_in * fabs( sin(angle) );
	int h_full = w_in * fabs( sin(angle) ) + h_in * fabs( cos(angle) );
